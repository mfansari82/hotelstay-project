...
namespace HotelStay.Infrastructure.Providers
{
    public class PremierStaysProvider : IHotelProvider
    {
        ...
        public PremierStaysProvider(ApplicationDbContext context)
        {
            _context = context;
        }
        ...
        public async Task<BookingResponseDto> BookAsync(BookingRequestDto request, CancellationToken cancellationToken = default)
        {
            ...
            var hotel = await _context.Hotels
                .FirstOrDefaultAsync(h => h.Name == request.HotelName && h.City == request.Destination && h.ProviderName == ProviderName, cancellationToken);
